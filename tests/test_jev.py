...
class ProjectGuardTests(unittest.TestCase):
    def test_equivalence_suggestion_cannot_resolve_structured_conflict(self):
        fake = Mock()
        fake.evaluate.return_value = {'answers': {'decision': {'choice': 'equivalent', 'confidence': 1}}, 'usage': {}}
        result = w.run(fake)
        self.assertEqual(result['structured_conflict']['status'], 'conflict')
        self.assertIsNone(result['structured_conflict']['answer'])
